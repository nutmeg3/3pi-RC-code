...
const int motorCompFactor = 1; //another compensation for slower motor
const int spinPower = 5;
...
void forward(int throttleSpeed, int steeringSpeed, int motorComp) {
    ...
    // steering left
    if ((throttleSpeed > 0) && (steeringSpeed < 0)){
        motors.setSpeeds(throttleSpeed + motorComp, (throttleSpeed*motorCompFactor) + abs(steeringSpeed));
