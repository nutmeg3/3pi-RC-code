...
float displacementDiscrepancy = 2;
    ...
    if(currentMillis > previousMillis + PERIOD){
        ...
        if ((abs(displacementLeftWheel - displacementRightWheel)) > displacementDiscrepancy){
            if (displacementLeftWheel < displacementRightWheel) {
                motors.setSpeeds(motorAdjustment, 0);
            }

            if(displacementLeftWheel > displacementRightWheel){
                motors.setSpeeds(0, motorAdjustment);
            }
        ...
        Serial.println("elapsed time in milliseconds");
